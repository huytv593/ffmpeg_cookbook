...
bash "install_yasm" do
	...
	cd yasm-1.3.0
	autoreconf -fiv
	./configure --prefix="/root/ffmpeg_build" --bindir="/usr/local/bin"
	make -j 4
	sudo make install
	make distclean
	EOH
...
bash "install_x264" do
  ...
  code <<-EOH
	...
	./configure --prefix="/root/ffmpeg_build" --bindir="/usr/local/bin" --enable-static
	make
	make install
	make distclean
	EOH
...
bash "install_libmp3lame" do
	...
	cd lame-3.99.5
	./configure --prefix="/root/ffmpeg_build" --disable-shared --enable-nasm
	make
	make install
	make distclean
	EOH
...
bash "install_libvorbis" do
	...
	cd libvorbis-1.3.4
	LDFLAGS="-L/root/ffmeg_build/lib"
	CPPFLAGS="-I/root/ffmpeg_build/include"
	./configure --prefix="/root/ffmpeg_build" --with-ogg="/root/ffmpeg_build" --disable-shared
	make
	make install
	make distclean
	EOH
...
bash "install_libvpx" do
	...
	cd libvpx-1.6.0
	./configure --prefix="/root/ffmpeg_build" --disable-examples
	make
	make install
	make distclean
	EOH
...
bash "install_ffmpeg" do
  ...
  environment "PKG_CONFIG_PATH" => "/root/ffmpeg_build/lib/pkgconfig"
	...
	cd ffmpeg-3.1.3
	./configure --prefix="/root/ffmpeg_build" --bindir="/usr/local/bin" --extra-cflags="-I/root/ffmpeg_build/include" --extra-ldflags="-L/root/ffmpeg_build/lib" --pkg-config-flags="--static" --enable-gpl --enable-nonfree --enable-libfdk-aac --enable-libfreetype --enable-libmp3lame --enable-libopus --enable-libvorbis --enable-libvpx --enable-libx264 --enable-libx265
	make
	make install
	make distclean
	EOH
